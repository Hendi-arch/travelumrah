...
export function Header() {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "fixed inset-0 z-[60] bg-white dark:bg-slate-950 px-6 py-6 lg:hidden transition-transform duration-300 ease-in-out",
      ...
      >
        ...
        <div className="mt-6 flow-root h-[calc(100vh-100px)] overflow-y-auto">
          <div className="-my-6 divide-y divide-slate-500/10">
                            ...
                            <div className={cn("mt-2 space-y-2 pl-6", openMobileSubmenu === item.name ? "block" : "hidden")}>
                                {item.children.map((child) => (
                                    <Link
                                        key={child.name}
                                        href={child.href}
                                        className="block rounded-lg py-2 pl-3 pr-3 text-sm font-semibold leading-7 text-slate-700 hover:bg-slate-50 hover:text-primary"
                                        onClick={() => setMobileMenuOpen(false)}
                                    >
                                        {child.name}
                                    </Link>
            ...
            <div className="py-6 space-y-3">
                 <Button className="w-full justify-start gap-2" variant="outline" onClick={() => window.location.href = 'tel:119'}>
                    <Phone className="h-4 w-4" />
                    Emergency: 119
                </Button>
                <Link href="/#book" className={buttonVariants({ variant: "default", className: "w-full" })} onClick={() => { setMobileMenuOpen(false); window.dispatchEvent(new Event('open-booking-modal')); }}>
                    Book Appointment
                </Link>
            </div>
          </div>
